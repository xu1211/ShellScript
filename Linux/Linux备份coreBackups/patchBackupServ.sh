...
#@@服务备份配置
servsCfg=$workPath/configs/$HostName/serv.cfg
if [ ! -f $appsCfg ]
then
	echo "$appsCfg配置文件不存在！"
	exit 0
...
scpPath=""
echo '******************************************************************************'
echo "Weblogic Domains Backup Start ..."
#@@读取配置文件
cat $servsCfg |while read line;
do
	#@@eg:#adapter=/app/qhdprod/adapter
	#@@应用名称
	servName=`echo $line | awk -F "=" '{ print $1 }'`
	#@@应用绝对路径
	servPath=`echo $line | awk -F "=" '{ print $2 }'`
	#@@备份文件名称
	arcName=$HostName-serv-$servName-$backUpDateTime

	if [ "$servName" = "" ]
	then
		echo "请检查$servsCfg配置文件有效性！"
		continue
	fi
	if [ "$servPath" = "" ]
	then
	...
	echo "Backup [$servName] Start, Path Value Is [$servPath] ..."
	...
	then
		#@@区间内应用文件变化量
		find $servPath -newer $workPath/$newerN1 ! -newer $workPath/$newerN2 ! -wholename 'A*' ! -wholename '*AdminServer*' ! -wholename '*qhdshare*' ! -wholename '*_TP_REPORT*' ! -wholename '*.ctl*' ! -wholename '*log*' ! -wholename '*.out*' ! -wholename '*.zip*' ! -wholename '*.DAT*' ! -wholename '*.tar.gz*' ! -wholename '*config.lok*' ! -wholename '*/hibernate/*' ! -wholename '*/finserver/par.txt*' -type f > "$bakFileTmp/$arcName.tmp"

		#@@查找结果为空即没有变化
		if [ ! -s $bakFileTmp/$arcName.tmp ]
		then
			#@@删除昨日备份，以实现变化全量备份
			echo "Delete [$lastBakFileDir/$HostName-serv-$servName] ..."
			rm -f $lastBakFileDir/$HostName-serv-$servName-*
		fi
		#@@新建当日备份
		echo "Create [$bakFileDir/$arcName.tar.gz] ..."
		rm -f $bakFileDir/$HostName-serv-$servName-*
		tar -zcf $bakFileDir/$arcName.tar.gz $servPath
	fi
	echo "Backup [$servName] End ..."
...
echo "Weblogic Domains Backup End ..."
echo '******************************************************************************'
exit 0